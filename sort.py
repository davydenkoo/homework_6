...
def make_transliteration(input_str: str):
    ...
    common_translit_dictionary = make_translit_dictionary(CYRILLIC_SYMBOLS, TRANSLIT_SYMBOLS)
...
def prepare_folder_structure(path_pointer: Path):
    ...
    for category in SORT_FILE_SETTINGS.keys():
        ...
        try:      
            ...
            log(f"[ERRO] >> An exception occurred when creating folder: {p}", 2)
            return {}
        else:
...
def if_file_exists(path_pointer: Path):
    ...
    while path_pointer.exists():
        log("{:40}: {}".format("[WARN] >> File/folder already exists", path_pointer))
...
def normalize(path_pointer: Path):
    ...
    log("{:40}: {}".format("[INFO] >> Original file/folder name", path_pointer))
    ...
    if new_f_name == f_name:
        ...
        try:
            ...
            log(f"[WARN] >> An exception occurred when renaming {path_pointer}. Skipping!", 2)
        else:
            log("{:40}: {}".format("[INFO] >> Renamed file/folder name", new_path_pointer), 2)
...
def processing_folder(
    ...
    if not path_pointer.is_dir():
        ...
        try:
            ...
            log(f"[WARN] >> An exception occurred when removing empty folder {path_pointer}. Skipping!", 2)
        else:
            log("{:40}: {}".format("[INFO] >> Remove empty folder", path_pointer), 2)

    else:
        path_pointer = normalize(path_pointer) if need_normalize_itself else path_pointer
        for item in path_pointer.iterdir():
            if item.is_dir():
                processing_folder(item, ignored_folders, True, folder_processing_data)
            else:
                    ...
                    if f_exten not in folder_processing_data["known_file_extens"] and \
                        ...
                        for category in SORT_FILE_SETTINGS.keys():
                            if f_exten in SORT_FILE_SETTINGS[category]:
                                folder_processing_data["known_file_extens"].append(f_exten)
                                folder_processing_data["files_by_categories"].setdefault(category, []).append(normalize_file)
                                break
                        else:
                            folder_processing_data["unknown_file_extens"].append(f_exten)
                            folder_processing_data["files_by_categories"].setdefault("other", []).append(normalize_file)
                    elif f_exten in folder_processing_data["known_file_extens"]:
                        for category in SORT_FILE_SETTINGS.keys():
                            if f_exten in SORT_FILE_SETTINGS[category]:
                                folder_processing_data["files_by_categories"][category].append(normalize_file)
                    elif f_exten in folder_processing_data["unknown_file_extens"]:
                        folder_processing_data["files_by_categories"]["other"].append(normalize_file)
                else:
                    folder_processing_data["files_by_categories"].setdefault("other", []).append(normalize_file)
            ...
            if category == "archives":
                ...
                if path_to_unpack.exists():
                    ...
                    try:
                        ...
                        log("{:40}: {}".format("[INFO] >> To folder name", path_to_unpack), 2)
    ...
    while flag:
        ...
        for folder in folders_tree.keys():
            ...
            if len(folders_tree[folder]) == 0:
                ...
                log("{:40}: {}".format("[INFO] >> Remove empty folder", folder), 2)
...
def main():
    ...
    if folder_pointer:
        ...
        if folders_for_sorting:
            ...
            folders_tree = build_folders_tree(folder_pointer, ignored_folders=folders_for_sorting)
